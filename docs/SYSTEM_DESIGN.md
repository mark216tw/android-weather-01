...
- 背景使用依天氣及晝夜切換的靜態漸層；前景生命週期內每 15 分鐘檢查一次資料年齡，達門檻才更新。
...
- 僅宣告網路及前景定位權限。
- 不使用背景定位、WorkManager 定時更新或持續位置監聽。自動更新只在 `STARTED` 前景生命週期中執行。
